create database query_builder_origin;

USE query_builder_origin;

create table escape_table (
	col_time time(6),
	col_date date,
	col_datetime datetime(6),
	col_datetime_zoned datetime(6)
);
...
CREATE PROCEDURE procedure_int(IN i_string varchar(20), OUT o_string varchar(20), IN i_integer int, OUT o_integer int, IN i_bool boolean)
BEGIN
...
CREATE PROCEDURE procedure_void(IN i_string varchar(20), OUT o_string varchar(20), IN i_integer int, OUT o_integer int, IN i_bool boolean)
BEGIN
  SELECT 42 INTO o_integer;
  SELECT 'someting' INTO o_string;
END;