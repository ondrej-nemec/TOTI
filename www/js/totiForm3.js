		...
		this.config.fields.push({
			...
			input: {
				...
				fields: [
					...
					{
						title: "Second In Pair",
						type: "text",
						name: "second-in-pair",
						id: "second-in-pair-id"
					}
				]
			}
		});
				...
				if (addButton !== null) {
					if (editable) {
						addButton.onclick = function() {
							object.dynamic[field.name](template.querySelectorAll('[name="toti-list-item"]').length)
						};
					} else {
						addButton.style.display = "none";
					}
				}
				this.dynamic[field.name] = function(position) {
					...
					if (useTemplate) {
						...
						removeButton.setAttribute("name", "remove");/* todo remove whole list */
	...
	*/
	...
	bindValues(values, form, createKey = null, originKey = null) {
		for (const[k, val] of Object.entries(values)) {
			var key = createKey === null ? k : createKey(k);
			var value = val;
			
			if (originKey !== null && this.dynamic[originKey] !== undefined) {
				this.dynamic[originKey](k);
			}
			if (value !== null && typeof value === "object") {
				this.bindValues(value, form, function(newKey) {
