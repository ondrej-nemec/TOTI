...
totiControl = {
	...
	data: {
		...
		loadGrid: function(config, initialLoad = false) {
			...
			var onFailure = function(xhr,status,error) {
				...
				$('#' + response.uniqueName + "-grid table tbody").html($('<div>').text(totiControlLang.messages.loadingError));
			};
			var onSuccess = function(response) {
				var body = $('#' + config.Name + "-control table tbody");
				body.html('');
				if (response.data.length === 0) {
					body.html($('<div>').text(totiControlLang.messages.noItemsFound));
					return;
				}
	...
	gui: {
		...
		gridPagging: function(config, actualPage, pageCount) {
			...
			pages.append($('<scan>').text(totiControlLang.pages.title));
			pages.append('&nbsp;');
			if (actualPage > 1) {
				pages.append($('<a>').attr("href", "").text(totiControlLang.pages.first).click(onPageClick(1, pages)));
				pages.append('&nbsp;');
			}
			if (actualPage > 2) {
				pages.append($('<a>').attr("href", "").text(totiControlLang.pages.previous).click(onPageClick(actualPage - 1, pages)));
				pages.append('&nbsp;');
			}
			...
			if (actualPage < pageCount) {
				pages.append($('<a>').attr("href", "").text(totiControlLang.pages.next).click(onPageClick(actualPage+1, pages)));
				pages.append('&nbsp;');
			}
			if ((actualPage + 1) < pageCount) {
				pages.append($('<a>').attr("href", "").text(totiControlLang.pages.last).click(onPageClick(pageCount, pages)));
				pages.append('&nbsp;');
			}
			pages.append('&nbsp;');
			...
			var actions = function() {
				...
				var button = $('<a>').attr("href", "").click(function() {
					...
					if (Object.keys(ids).length === 0) {
						totiControl.showMessage(config.Name, totiControlLang.messages.noItemsSelected, "toti-control-flash error");
						return false;
					}
